...
String.prototype.parseColor = function () {
    let color = new Color(this as string);
...
export const cocosUtil = {
    ...
    addKeyDownListener(cb : any, sender : any) {
     ...
     */
    drawCurvePath(node : Node, start: number[], end: number[], curveness: number, percent: number, delay = 0) {
    ...
    blNode(node : Node, cb : (n : Node) => void){
        cb(node);
